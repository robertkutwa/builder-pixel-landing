import { z } from "zod";
import { WEIGHT_LIMITS, WEIGHT_PRICES, } from "../../shared/api.js";
// Mock database
const mockParcels = [
    {
        id: "parcel-1",
...
function calculateWeightCategory(weight) {
    if (weight <= WEIGHT_LIMITS.light.max)
        return "light";
    if (weight <= WEIGHT_LIMITS.medium.max)
        return "medium";
    if (weight <= WEIGHT_LIMITS.heavy.max)
        return "heavy";
    return "extra_heavy";
}
...
function calculateCost(distance, weightCategory) {
...
export const handleGetQuote = async (req, res) => {
    try {
        ...
        const distance = calculateDistance(pickupLocation.latitude, pickupLocation.longitude, deliveryLocation.latitude, deliveryLocation.longitude);
        const weightCategory = calculateWeightCategory(size.weight);
        const { baseCost, weightCost, totalCost } = calculateCost(distance, weightCategory);
        const quote = {
            baseCost,
            weightCost,
...
export const handleCreateParcel = async (req, res) => {
    try {
        ...
        const distance = calculateDistance(validatedData.pickupLocation.latitude, validatedData.pickupLocation.longitude, validatedData.deliveryLocation.latitude, validatedData.deliveryLocation.longitude);
        ...
        const newParcel = {
            ...
            estimatedDeliveryAt: new Date(Date.now() + Math.round((distance / 30) * 60 * 60 * 1000)).toISOString(),
        };
...
export const handleGetParcels = async (req, res) => {
    try {
        const user = req.user;
...
export const handleGetParcel = async (req, res) => {
    try {
        const user = req.user;
        const { id } = req.params;
...
export const handleUpdateParcelStatus = async (req, res) => {
    try {
        ...
        const statusUpdate = {
            ...
            message: validatedData.message || `Status updated to ${validatedData.status}`,
            updatedBy: user.id,
            updatedByName: `${user.firstName} ${user.lastName}`,
            timestamp: new Date().toISOString(),
        };
        ...
        const updatedParcel = {
            ...
            updatedAt: new Date().toISOString(),
            deliveredAt: validatedData.status === "delivered"
...
export const handleCancelParcel = async (req, res) => {
    try {
        const user = req.user;
        const { id } = req.params;
        const parcelIndex = mockParcels.findIndex((p) => p.id === id);
