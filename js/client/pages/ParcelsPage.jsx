...
import { Package, Plus, Search, Filter, Eye, X, MapPin, CheckCircle, XCircle, AlertCircle, Truck, RefreshCw, SortAsc, SortDesc, ArrowUpDown, } from "lucide-react";
...
export default function ParcelsPage() {
    ...
    const filterAndSortParcels = () => {
        ...
        // Apply search filter
        if (searchTerm) {
            filtered = filtered.filter((parcel) => parcel.trackingNumber
                .toLowerCase()
    ...
    const cancelParcel = async () => {
        ...
        try {
            setCancelling(true);
            setError(null);
            const response = await authenticatedFetch(`/api/parcels/${selectedParcel.id}`, {
                method: "DELETE",
            });
            const data = await response.json();
            if (data.success) {
                // Update the parcel in local state
                setParcels((prev) => prev.map((p) => p.id === selectedParcel.id ? { ...p, status: "cancelled" } : p));
    ...
    const getSortIcon = (field) => {
        ...
        return sortOrder === "asc" ? (<SortAsc className="w-4 h-4 text-primary"/>) : (<SortDesc className="w-4 h-4 text-primary"/>);
    };
    // Pagination
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
        ...
        <div className="mt-4 lg:mt-0 flex space-x-2">
          ...
          <Button onClick={() => navigate("/parcels/new")} className="btn-primary">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <Input id="search" placeholder="Search parcels..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/>
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className="space-y-4">
            ...
            </div>) : currentParcels.length > 0 ? (<>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleSort("trackingNumber")}>
                ...
                <TableBody>
                  {currentParcels.map((parcel) => (<TableRow key={parcel.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/parcels/${parcel.id}`)}>
                      ...
                      <TableCell>
                        <Badge variant="secondary" className={cn("text-xs", getStatusBadgeVariant(parcel.status))}>
                      ...
                      <TableCell>
                        <div className="w-24">
                          <Progress value={getProgressValue(parcel.status)} className="h-2 mb-1"/>
                      ...
                      <TableCell>
                        <div className="flex space-x-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          {canCancelParcel(parcel) && (<Button variant="outline" size="sm" onClick={() => {
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={cancelParcel} disabled={cancelling}>
              {cancelling ? (<>
