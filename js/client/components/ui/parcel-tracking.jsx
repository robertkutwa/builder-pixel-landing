...
import { MapPin, Navigation, Clock, Truck, RefreshCw, CheckCircle, AlertCircle, XCircle, Route, Maximize2, Minimize2, } from "lucide-react";
...
const GoogleMap = ({ pickup, delivery, current, className }) => {
    ...
    return (<div className={cn("relative bg-muted rounded-lg overflow-hidden", className)}>
      {/* Mock Map Interface */}
      <div ref={mapRef} className={cn("bg-gradient-to-br from-blue-50 to-green-50 dark:from-blue-950 dark:to-green-950 flex items-center justify-center text-muted-foreground border-2 border-dashed border-muted-foreground/20", isExpanded ? "h-96" : "h-48")}>
        <div className="text-center space-y-2">
          <MapPin className="w-8 h-8 mx-auto"/>
          <div className="text-sm font-medium">Route Visualization</div>
          <div className="text-xs">
            {Math.round(Math.sqrt(Math.pow(delivery.lat - pickup.lat, 2) +
            Math.pow(delivery.lng - pickup.lng, 2)) * 111)}{" "}
            km route
          </div>
          <div className="flex items-center justify-center space-x-4 text-xs">
            ...
            {current && (<div className="flex items-center space-x-1">
      ...
      <div className="absolute top-2 right-2 flex space-x-2">
        <Button variant="secondary" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="h-8 w-8 p-0 bg-background/80 backdrop-blur">
...
export function ParcelTracking({ parcel, onRefresh, showMap = true, className, }) {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <Badge variant="secondary" className={cn("text-sm", getStatusColor(parcel.status))}>
                {STATUS_LABELS[parcel.status]}
              </Badge>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Progress value={getProgressValue(parcel.status)} className="h-3"/>
            </div>
            ...
            {isActiveParcel && (<Alert className="border-primary/20 bg-primary/5">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            .map((status, index) => (<div key={status.id} className="flex space-x-4">
                  <div className="flex flex-col items-center">
                    <div className={cn("w-3 h-3 rounded-full", index === 0 ? "bg-primary" : "bg-muted")}/>
                    {index < parcel.statusHistory.length - 1 && (<div className="w-px h-8 bg-muted mt-2"/>)}
                  </div>
                  <div className="flex-1 pb-4">
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      {index === 0 && (<Badge variant="outline" className="text-xs">
                          Latest
                        </Badge>)}
                    ...
                    {status.location && (<div className="text-xs text-muted-foreground mb-1">
