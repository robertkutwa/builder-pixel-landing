...
import { Truck, Package, User, LogOut, Menu, X, Home, BarChart3, Settings, } from "lucide-react";
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            const canShow = item.public || (item.requireAuth && user) || !item.requireAuth;
            if (!canShow)
                return null;
            return (<Link key={item.href} to={item.href} className={cn("flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors", isActive
          ...
          <div className="hidden md:flex items-center space-x-4">
            {user ? (<DropdownMenu>
                ...
                <Button onClick={() => navigate("/register")} className="text-sm btn-primary">
                  Get Started
                </Button>
              </div>)}
          </div>
        ...
        {isMobileMenuOpen && (<div className="md:hidden py-4 border-t">
            <div className="flex flex-col space-y-2">
              {navigationItems.map((item) => {
                ...
                if (!canShow)
                    return null;
                return (<Link key={item.href} to={item.href} onClick={() => setIsMobileMenuOpen(false)} className={cn("flex items-center space-x-3 px-3 py-3 rounded-md text-sm font-medium transition-colors", isActive
                        ? "bg-primary/10 text-primary"
                        : "text-muted-foreground hover:text-foreground hover:bg-muted/50")}>
